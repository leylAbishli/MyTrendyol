...
import com.example.mytrendyol.favorites.OnFavoriteChangedListener
import com.example.mytrendyol.main.adapters.MainAdvertisementAdapter
...
@AndroidEntryPoint
class MainFragment : Fragment(), OnFavoriteChangedListener {
    ...
    private fun filterProductsByCategory(category: String, isChecked: Boolean, modelType: String) {
        val filteredList = when (modelType) {
            ...
            "FlashProductModel" -> {
                ...
                emptyList() // Veya uygun bir varsayılan işlem yapabilirsiniz
            }
        }
    ...
    private fun filterProductsByName(query: String) {
        ...
        val filteredMainList = productList.filter { product ->
            product.productName?.trim()?.toLowerCase()?.contains(trimmedQuery) ?: false
        }
